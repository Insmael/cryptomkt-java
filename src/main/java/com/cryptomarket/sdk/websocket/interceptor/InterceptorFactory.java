...
public class InterceptorFactory {
  ...
  public static <T> Interceptor newMapStringListOfChanneledWSResponseObject(Callback<Map<String, List<T>>> callback,
      Class<T> cls) {
    return new Interceptor() {
      @Override
      public void makeCall(WSJsonResponse response) {
  ...
  public static <T> Interceptor newOfChanneledWSResponse(Callback<Map<String, T>> callback, Class<T> cls) {
    return new Interceptor() {
      @Override
      public void makeCall(WSJsonResponse response) {
        ErrorBody error = response.getError();
  ...
  public static Interceptor ofSubscriptionResponse(Callback<List<String>> callback) {
    return new Interceptor() {
      @Override
      public void makeCall(WSJsonResponse response) {
        ErrorBody error = response.getError();
